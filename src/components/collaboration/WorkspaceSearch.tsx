...
export const WorkspaceSearch = ({ open, onOpenChange, labId, onSelect }: WorkspaceSearchProps) => {
    ...
    }>({ messages: [], channels: [], files: [], projects: [] });

    const performSearch = useCallback(
        debounce(async (searchQuery: string) => {
            if (!searchQuery.trim()) {
                setResults({ messages: [], channels: [], files: [], projects: [] });
                setLoading(false);
                return;
            }
    ...
    const hasResults = results.messages.length > 0 || results.channels.length > 0 || results.files.length > 0 || results.projects.length > 0;
                ...
                <ScrollArea className="max-h-[60vh]">
                    {loading ? (
                        <div className="p-12 text-center space-y-4">
                            ...
                            <p className="text-sm text-muted-foreground animate-pulse">Searching Lab-IQ Workspace...</p>
                        </div>
                    ) : query && !hasResults ? (
                        ...
                        <div className="p-8">
                            <p className="text-[10px] uppercase font-bold text-muted-foreground mb-4 tracking-widest flex items-center gap-2">
                                <Command className="h-3 w-3" /> Quick Navigation
                            </p>
                        ...
                        <div className="p-4 space-y-6 pb-8">
                            ...
                            {results.channels.length > 0 && (
                                <div className="space-y-2">
                                    <h4 className="text-[10px] uppercase font-bold text-muted-foreground px-2 tracking-widest">Channels</h4>
                            ...
                            {results.messages.length > 0 && (
                                <div className="space-y-2">
                                    <h4 className="text-[10px] uppercase font-bold text-muted-foreground px-2 tracking-widest">Messages</h4>
                                        ...
                                        >
                                            <div className="flex items-center justify-between mb-1">
                                                <div className="flex items-center gap-2">
                                                    <span className="text-xs font-bold">{msg.user?.display_name}</span>
                                                    <span className="text-[10px] text-muted-foreground">{format(new Date(msg.created_at), 'MMM d')}</span>
                                                </div>
                                                <Badge variant="outline" className="text-[8px] h-4">#{msg.channel_id.substring(0, 8)}</Badge>
                                            </div>
                                            <p className="text-sm text-foreground/80 line-clamp-2 group-hover:text-foreground">{msg.content}</p>
                                        </div>
                                    ))}
                                </div>
                            )}
                            ...
                            {results.files.length > 0 && (
                                <div className="space-y-2">
                                    <h4 className="text-[10px] uppercase font-bold text-muted-foreground px-2 tracking-widest">Files</h4>
                                    {results.files.map(file => (
                                        <div key={file.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-primary/10 cursor-pointer group transition-all">
                                            <FileText className="h-4 w-4 text-blue-500" />
                                            <div>
                                                <p className="text-sm font-medium">{file.name}</p>
                                                <p className="text-[10px] text-muted-foreground">{file.category}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
