...
import { HeartPulse, Home, Upload, Brain, BarChart3, FlaskConicalIcon, Boxes, Users, FileText, Cpu, Database, ChevronLeft, ChevronRight, Settings, LayoutDashboard, Sparkles, Microscope } from "lucide-react";
...
const Sidebar = () => {
  ...
  const navItems = [
    ...
    { path: "/analysis", label: "AI Analysis", icon: Microscope },
    ...
    <>
      <div
        className={cn(
          ...
          isCollapsed ? "w-0 -translate-x-full border-none opacity-0 overflow-hidden pointer-events-none p-0 m-0" : "w-64 translate-x-0 opacity-100"
        )}
      >
        {/* Header */}
      ...
      {isCollapsed && (
        <Button
          ...
          size="icon"
          onClick={toggleSidebar}
          className="fixed left-4 bottom-4 z-50 h-10 w-10 rounded-xl shadow-lg bg-background/80 backdrop-blur border border-border hover:bg-muted animate-in fade-in slide-in-from-left-4 duration-300 hidden md:flex"
        >
